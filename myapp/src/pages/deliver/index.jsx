...
import { fetchDeliver, fetchOrderImages } from "../../apis";
...
const Deliver = () => {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      const orderId = await fetchDeliver(orderData);
      console.log("订单提交成功:", orderId);

      const imageRes = await fetchOrderImages(files[0].url, orderId);
    ...
    <>
      {useAuthStore.getState().needLogin ? (
        <View className="mt-32">
          <Empty>
            <Empty.Image className="custom-empty__image" src={UnLogin} />
            <Empty.Description>授权登录后使用完整功能</Empty.Description>
            <View className="mt-4">
              <Button
                shape="round"
                className="bottom-button w-40"
                style={{
          ...
          <Form ref={formRef} onSubmit={onSubmit}>
            ...
            <Cell.Group inset>
              <Field
                label="取件码"
                name="expressId"
                rules={[{ required: true, message: "请填写取件码" }]}
              >
