...
const OrderInfo = () => {
  ...
  useEffect(() => {
    ...
    const orderData = {
      ...
      comment: comment ? decodeURIComponent(comment) : "请轻拿轻放，易碎物品",
      payment: {
        amount: "3.00",
        method: "微信支付",
        time: dayjs(Number(orderTime))
          .add(1, "minute")
          .format("YYYY-MM-DD HH:mm:ss"),
        status: "已支付",
      },
  ...
  const getStatusColor = (state) => {
    ...
    if (
      state === STEP_STATES.STEP1.REJECTED ||
      state === STEP_STATES.STEP2.FAILED
    )
      return "danger";
      ...
      <View className="mx-4 mt-4 rounded-xl overflow-hidden">
        ...
        >
          <View className="flex items-center mb-3">
            <Tag
              color={getStatusColor(orderInfo.stepState)}
              className="mr-2 h-6"
            >
          ...
          {orderInfo.stepState >= STEP_STATES.STEP1.ACCEPTED &&
            orderInfo.stepState < STEP_STATES.STEP3.DELIVERED && (
              ...
              <View className="flex w-full py-2">
                ...
                <View className="ml-3 flex-1">
                  <View className="flex justify-between">
                    <Text className="font-medium">
                      {orderInfo.courierInfo.name}
                    </Text>
                    <View className="flex items-center">
                      <StarOutlined className="text-yellow-500 mr-1" />
                      <Text className="text-yellow-500">
                        {orderInfo.courierInfo.favor}
                      </Text>
                    </View>
                  </View>
                  <Text className="text-sm text-gray-500 mt-1">
                    {orderInfo.courierInfo.phone}
                  </Text>
