...
###### 3.26
懒加载的 scrollview 中的 h-screen 属性和 backtop 功能冲突

更换技术选型，不再使用taro 原生 scrollview 改用 taroify 的 list 解决了 backtop 的 bug
...
客户端没有强制绑定电话，如果商户在客户端注册了账号但是没有绑定电话，那么在客户端用电话注册，则会出现一个人两个账号的情况，这是不应该的

暂时想到的解决办法是在注册页面强制绑定电话号码和用户id，没有id则需要先去客户端注册账号

###### 4.5
zustand
fetch

由于登录问题，首先跳过几步，先去写页面和逻辑，之后再补
- request请求拦截器封装
- zustand管理token
- token持久化
- 请求拦截器注入token
...
流水分析基本完成
v0真神
考虑到一个问题，如果有多个骑手，这订单应该绑定骑手id，不应该可以操控他人
那么流水应该具备查看全部流水和个人流水的功能
